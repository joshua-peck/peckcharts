...
TITLE = "Home Prices have dropped 44% relative to M2 money supply since the GFC"
SUBTITLE = "Current reading of 0.0146 is 24% below the long-term average"
...
CSV = Path(__file__).parent / "datafiles" / "WM2NS_CSUSHPINSA.csv"
...
def main():
    ...
    png = save_for_social(fig, "home_prices_vs_m2", path=str(OUT))
    pdf = save_for_social(fig, "home_prices_vs_m2", path=str(OUT),
                          fmt="pdf")
    print(f"wrote {png}")
    print(f"wrote {pdf}")
