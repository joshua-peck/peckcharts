...
TITLE = "Mortgage spreads are returning to long-term averages after COVID-era uncertainty wanes"
SUBTITLE = "Current level of 2% is 0.24% from the long-term average"
...
CSV = Path(__file__).parent / "datafiles" / "MORTGAGE30US_DGS10.csv"
...
def main():
    ...
    for level, label in [
        ...
        ax.axhline(level, color=colors.CREAM, linewidth=0.7, linestyle=":",
                   alpha=0.5)
        ax.text(last_x, level, f"  {label} {level:.2f}%",
    ...
    png = save_for_social(fig, "mortgage_spread_smoothed", path=str(OUT))
    pdf = save_for_social(fig, "mortgage_spread_smoothed", path=str(OUT),
                          fmt="pdf")
    print(f"wrote {png}")
    print(f"wrote {pdf}")
